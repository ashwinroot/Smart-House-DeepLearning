...
baseline = baseline()
print(baseline)
for n_hidden in n_hidden_layer:
    print("Number of hidden layers : " , n_hidden)
    n_input = len(X[0]) - 1
    network = init_network(n_input, n_hidden, 1)
    Trainer(network, X, occupancy, 1, 0.01,25,64)
    for x in range(0,25):
        baseline_grp.append(baseline)
    print("\n Train Accuracy over epoches : ", train_accuracy_grp, '\n Train Average Accuracy : ', np.mean(accuracy))
    print('\n Test Accuracy over epoches: ',accuracy, '\n Test Average accuracy : ' ,np.mean(accuracy))
    global_train.append(np.max(train_accuracy_grp))
    global_test.append(np.max(accuracy))
print("Global train : ",global_train,"Global test : ", global_test )
fig1 = plt.figure("Over the hidden layers : ")
plt.plot(n_hidden_layer,global_train,'r',label='train')
plt.plot(n_hidden_layer,global_test,'b',label='test')
plt.ylabel('Accuracy')
